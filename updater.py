# tools/updater.py
# -*- coding: utf-8 -*-
import os, sys, time, json, shutil, hashlib, zipfile, tempfile, subprocess, ctypes
import requests
import json
...
# Domyślna ścieżka instalacji (możesz zmienić na Program Files, jeśli chcesz)
INSTALL_DIR = os.path.expanduser(rf"{os.environ.get('LOCALAPPDATA')}\{APP_NAME}")
APP_EXE = os.path.join(INSTALL_DIR, "Piotrflix.exe")

# Plik z lokalną wersją (generowany przy instalacji / buildzie)
LOCAL_VERSION_FILE = os.path.join(INSTALL_DIR, "version.json")

# GitHub API – repo z wydaniami
GITHUB_OWNER = "pkawa95"   # TODO: podmień
GITHUB_REPO  = "PiotrFlix"   # TODO: podmień
...
def _schedule_replace_on_reboot(temp_path: str, dest_path: str) -> bool:
    try:
        ...
        # najpierw zarejestruj docelową podmianę (temp -> dest)
...
def replace_file_atomic(dest: str, data: bytes) -> None:
    ...
    try:
        ...
        # plik zablokowany – spróbuj zaplanować podmianę po restarcie
        scheduled = _schedule_replace_on_reboot(tmp, dest)
        if not scheduled:
            # nie udało się – zostaw jako .updtmp obok
            # (użytkownik będzie musiał ubić proces i odpalić aktualizację ponownie)
            pass
    except Exception:
        # cokolwiek innego – zostaw .updtmp obok
        pass
...
def try_shutdown(status_cb=None):
    try:
        ...
        pass
    # czekaj aż /status przestanie odpowiadać
    t0 = time.time()
    ...
    def run(self):
        try:
            ...
            with tempfile.TemporaryDirectory() as td:
                ...
                try:
                    with zipfile.ZipFile(zip_path, "r") as z:
                        ...
                        for i, m in enumerate(members, 1):
                            rel = m.filename
                            dest = os.path.join(self.install_dir, rel)
                ...
                # 6) Zapisz nową wersję
                try:
                    ensure_dir(os.path.dirname(LOCAL_VERSION_FILE))
...
def _find_static_dir():
    # Spróbuj w katalogu instalacji, a potem obok updatera (dev)
    candidates = [
        os.path.join(INSTALL_DIR, "static"),
        os.path.join(os.path.dirname(sys.argv[0]), "static"),
...
class UpdaterWindow(QtWidgets.QWidget):
    ...
    def _restart_app_and_exit(self):
        ...
        try:
            subprocess.Popen([APP_EXE], cwd=INSTALL_DIR, close_fds=True)
        except Exception:
            pass
        self._quit_now()
...
def main():
    ...
    win = UpdaterWindow()
    win.show()
    QtCore.QTimer.singleShot(50, win.start)  # odpalenia pracy po pokazaniu okna
    return app.exec()
